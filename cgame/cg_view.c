...
//[CoOp]
/* replaced by new CoOp camera code.
...
{
	//[CoOp]
	//now use CoOp's camera code for the camera shake
...
//[CoOp]
//replaced with CoOp version of same code
/*
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	{	
		...
		//button presses skip the cutscene.
		if(cmd.buttons & BUTTON_USE || cmd.buttons & BUTTON_ATTACK)
		{//do nothing so we will skip the cutscene later
		}
		else if(trap_CIN_RunCinematic(CinematicNum))
		{//roq file running
		...
		CinematicNum = 0;
		InCinematic = qfalse;
		trap_CIN_StopCinematic(CinematicNum);
		trap_SendClientCommand("EndCinematic");
	}
	//[/ROQFILES]
