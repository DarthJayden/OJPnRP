...
#include "cg_local.h"
#include "../game/bg_public.h" //Jayden.
...
static void C_G2Mark(void)
{
	...
	{ //hit someone with a ghoul2 instance, let's project the decal on them then.
		...
		if(!(cent->currentState.NPC_class == CLASS_VEHICLE 
			&& cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo->ResistsMarking))
		{
...
//	{ &cg_trueLightning, "cg_trueLightning", "0.0", CVAR_ARCHIVE},
	...
	//[CoOp]
	{ &ui_menubrief, "ui_menubrief", "0", CVAR_ROM|CVAR_INTERNAL}, //used for opening mission briefings
	//[/CoOp]
...
//[/Movie Sabers]
	...
	{ &ojp_holsteredweapons, "ojp_holsteredweapons", "2", CVAR_ARCHIVE },
...
//	CG_LoadingString( "collision map" );
	...
	//[MapURLs]
	if(cgs.mapURL[0] != '\0')
	{//check to see if the map exists, if it doesn't, drop out with a message that uses the server's mapURL cvar to
		//indicate where the players can find that map.
		fileHandle_t	f;
		trap_FS_FOpenFile(cgs.mapname, &f, FS_READ);
		if(!f)
		{
			CG_Error("Unable to find map %s.\nDownload URL:\n%s\n", cgs.mapname, cgs.mapURL);
		}
		trap_FS_FCloseFile(f);
	}
	//[/MapURLs]
