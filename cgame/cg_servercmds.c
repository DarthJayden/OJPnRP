...
static void CG_ParseTeamInfo( void ) {
	...
	int		client;
	//[BugFix34]
	//changed the function to handle the new "delta" based team overlay updates.
	int		transNumber;  //number of players updated in this data packet.
	int		x;
	...
	//[/BugFix34]
		...
		//[BugFix34]
		//figure out if this player is on the overlay or not yet.
		for(x = 0; x < numSortedTeamPlayers; x++)
		{
			if(sortedTeamPlayers[x] == client)
			{//found ourself
				break;
			}
		}
		if(x == numSortedTeamPlayers)
		{//this player isn't in the sorted listed, add them
...
*/
//[SIEGECVARFIX]
extern void siege_Cvar_Set( char *cvarName, char *value );
//[/SIEGECVARFIX]
...
void CG_KillCEntityG2(int entNum)
{
	...
	if (ci)
	{
		...
		while (j < MAX_SABERS)
		{
			...
			//[VisualWeapons]
			if (ci->ghoul2HolsterWeapons[j] && trap_G2_HaveWeGhoul2Models(ci->ghoul2HolsterWeapons[j]))
			{
				trap_G2API_CleanGhoul2Models(&ci->ghoul2HolsterWeapons[j]);
...
static void CG_ServerCommand( void ) {
	...
	//[CoOp]
	if ( !strcmp( cmd, "briefmenu" ) )
	{//open up the briefing menu for the next map.
		//grab the briefing menu file.
		InBriefing = qtrue;
		trap_Cvar_Set("ui_menubrief", CG_Argv(1));
		trap_OpenUIMenu(UIMENU_BRIEFING);
		return;
	}
	//[/CoOp]
