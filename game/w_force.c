...
/*if ( self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2 )
	...
	{//tube
		...
		for (e = 0; e < numListedEntities; e++)
		{
			...
			//Воспользуемся школьными методами. 
			VectorMA(self->client->ps.origin, 2048, forward, end); //Конец линии взгляда.
			//У нас есть center - начало и end - конец некоего отрезка. Нужно проверить расстояние до него от ent_org
			float ab, cb, ac, perimeter, distance; //ИЩЕМ РАССТОЯНИЕ КАК ВЫСОТУ ТРЕУГОЛЬНИКА ABC xD
