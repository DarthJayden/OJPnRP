...
//*** IMPORTANT!!! *** vehFields table correponds to this structure!
	...
	//ROP VEHICLE_IMP START
	qboolean	VehicleMovementModesActive;

	int			MBFstopprimaryfiring;
	int			MBFstopaltfiring;
	int			MBFdisableshields;

	qboolean	CantKnockoutShields;
	qboolean	AllWeaponsDoDamageToArmor;
	qboolean	AllWeaponsDoDamageToShields;
	qboolean	ResistsMarking;
	float		SpeedMultiplierForRamDamage;
	//ROP VEHICLE_IMP END

	//[CloakingVehicles]
	qboolean	CloakingDevice;
	int			CloakingDeviceOnSound;	//sound played when cloaking device is turned on.
	int			CloakingDeviceOffSound;	//sound played when cloaking device is turned off.
	//[/CloakingVehicles]
...
//[/Linux]
{
	...
	//ROP VEHICLE_IMP START
	int			m_iDisabledShieldStore;	//Keeps a track of shield strength while disabled
	//ROP VEHICLE_IMP END
	...
	//ROP VEHICLE_IMP START
	vehicleMode_t	eCurrentMode;	//THe current mode the vehicle is in
	vehicleMode_t	eChangedModeFrom;
	int			nTransAnim;			//
	int			nTransAnimTime;
	qboolean	bTransAnimFlag;

	//qboolean	bTurningOnSpot;
