...
static bool Update( Vehicle_t *pVeh, const usercmd_t *pUmcd )
{
	...
	//ROP VEHICLE_IMP START
	bShieldsDisabled = pVeh->m_pVehicleInfo->MBFdisableshields & pVeh->eCurrentMode;
	if(bShieldsDisabled)
	{
		//Use the stored shield value
		iShieldCheck = pVeh->m_iDisabledShieldStore;
	}
	else
	{
		//Use the actual shield value
		iShieldCheck = parentPS->stats[STAT_ARMOR];
	}

	//increment shields for rechargable shields
	if ( pVeh->m_pVehicleInfo->shieldRechargeMS
		&& (iShieldCheck > 0 || pVeh->m_pVehicleInfo->CantKnockoutShields) //still have some shields left
		&& iShieldCheck < pVeh->m_pVehicleInfo->shields//its below max
		&& pUmcd->serverTime-pVeh->lastShieldInc >= pVeh->m_pVehicleInfo->shieldRechargeMS )//enough time has passed
	{
		iShieldCheck++;
		pVeh->lastShieldInc = pUmcd->serverTime;
		if ( iShieldCheck > pVeh->m_pVehicleInfo->shields )
		{
			iShieldCheck = pVeh->m_pVehicleInfo->shields;
		}

		if(bShieldsDisabled)
		{
			//OK - put the newly updated armor value back in store, and
			//put the amour statistic back to 0
			pVeh->m_iDisabledShieldStore = iShieldCheck;
			parentPS->stats[STAT_ARMOR] = 0;
		}
		else
		{
			parentPS->stats[STAT_ARMOR] = iShieldCheck;
		}
...
//[CloakingVehicles]
qboolean DrivingCloakableVehicle(gentity_t *self)
{//checks to see if the given player is ridding a vehicle that is cloakable.
	gentity_t *vehEnt = &g_entities[self->client->ps.m_iVehicleNum];
	if(vehEnt->m_pVehicle && vehEnt->m_pVehicle->m_pVehicleInfo->CloakingDevice)
	{
		return qtrue;
	}
	return qfalse;
...
extern void G_SoundIndexOnEnt( gentity_t *ent, int channel, int soundIndex );
void G_ToggleVehicleCloak(playerState_t *ps)
{//toggle the cloak powerup for this vehicle
	gentity_t *vehicle = &g_entities[ps->clientNum];

	if(ps->powerups[PW_CLOAKED])
	{//we can always decloak.
		ps->powerups[PW_CLOAKED] = 0;

		if ( vehicle->m_pVehicle->m_pVehicleInfo->CloakingDeviceOffSound )
		{//play cloak toggle sound if we have one.
			G_SoundIndexOnEnt( vehicle, CHAN_AUTO, 
				vehicle->m_pVehicle->m_pVehicleInfo->CloakingDeviceOffSound );
		}
	}
	else if(vehicle->m_pVehicle->m_iArmor > .4 * vehicle->m_pVehicle->m_pVehicleInfo->armor)
	{//can't cloak unless we have more than %40 health.
		ps->powerups[PW_CLOAKED] = Q3_INFINITE;
		
		if ( vehicle->m_pVehicle->m_pVehicleInfo->CloakingDeviceOnSound )
		{//play cloak toggle sound if we have one.
			G_SoundIndexOnEnt( vehicle, CHAN_AUTO, 
				vehicle->m_pVehicle->m_pVehicleInfo->CloakingDeviceOnSound );
		}
	}
}
//[/CloakingVehicles]
