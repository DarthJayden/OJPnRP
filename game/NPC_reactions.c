...
//	}
	...
	//[CoOp]
	/* RAFIXME - this is weird stuff that needs to be fixed for MP.
		//I think this might be for setting the player's look target.
...
gentity_t *G_CheckControlledTurretEnemy(gentity_t *self, gentity_t *enemy, qboolean validate )
{
		...
		*//*
	{
		...
		{//return the controller of the eweb/emplaced gun
			if (validate==qfalse || !self->client || ValidEnemy(self))
			{
